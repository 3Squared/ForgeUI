<template>
  <b-dropdown @hide="revert" ref="dropdown" :variant="hasChanges ? 'primary' : 'outline-primary'" class="forge-table-customise-columns-button">
    ...
    <b-form-checkbox-group id="selectedColumns-group" v-model="selectedFields" name="selectedColumns" class="mt-3">
      <b-list-group>
        <draggable v-model="availableFields" group="columns" handle=".handle" ghost-class="ghost">
          <b-list-group-item class="p-2 d-flex align-items-center" :key="getColumnKey(field)" v-for="field in availableFields.filter(f => canFieldBeHidden(f))">
...
import { startCase, arraysEqual } from '../../../helpers';
