<template>
  <div key="_bv-icons-table_" class="bv-icons-table notranslate" role="group" aria-labelledby="bv-icons-table-title">
    <b-row class="">
      <b-col cols="12">
    ...
    <div id="bv-icons-table-result">
      ...
      >
        ...
        >
          <b-card bg-variant="light" class="mx-auto border-0" no-body :class="{ iconHoverSize: icon.isHovering }" @click="clickedIcon(icon)">
    ...
    <p class="text-center">
      Use the search to search all {{ allIcons.length }} icons, or view all at
      <a href="https://bootstrap-vue.org/docs/icons#icons" target="_blank">Bootstrap Icons</a>
    </p>
  </div>
</template>
...
export default Vue.extend({
  ...
  beforeMount() {
    this.loadedIcons = [
      ...forgeIcons.sort().map((forgeIcon: string) => {
        return {
          component: forgeIcon,
          name: forgeIcon
            .replace(/^BIcon/, "")
