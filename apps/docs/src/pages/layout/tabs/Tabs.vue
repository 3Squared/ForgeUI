<template>
  <div>
    ...
    <p>
      ...
      <a class="link" target="_blank" href="https://bootstrap-vue.org/docs/components/tabs">
        <strong>Bootstrap Vue documentation</strong>
      </a>
      .
    </p>
    <playground :options="options" :config="config.value" :code="code" @reset="reset">
      <template #header>
...
const code = computed(
  () => `<b-tabs${tabsConfig.propVals.value.length > 0 ? " " + tabsConfig.propVals.value.join(" ") : ""}>
