...
describe("Table Column Builder", () => {
  ...
  it("both ways of defining columns should return same structure", () => {
    const builder = new ForgeColumnBuilder<Animal>()
      .addColumn("name", (col) => col.disableSorting())
      .addMiscColumn("identity.name", (col) => col.disableSorting());
    ...
    const builder2 = new ForgeColumnBuilder<Animal>()
      .addColumn(
        (c) => c.name,
        (col) => col.disableSorting()
      )
      .addColumn(
        (k) => k.identity.name,
        (col) => col.disableSorting()
      );
    expect(JSON.stringify(builder2.build())).toBe(
      JSON.stringify([
        {
          key: "name",
          sortable: false
