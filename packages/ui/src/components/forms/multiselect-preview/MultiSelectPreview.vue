<template>
  <div :class="orientation === 'horizontal-reversed' ? 'row flex-row-reverse' : 'row'">
    <div :class="orientation === 'vertical' ? 'col-12 mb-1' : 'col'">
      <forge-multi-select :multiple="true" :limit-text="limitText" :value="selectedItems" :options="items" @input="update">
        <template #tag>{{ "" }}</template>
        <template #selection>
          <span v-if="selectedItems.length > 0" class="pl-1">{{ selectedItems.length }} items selected</span>
        </template>
        <slot v-for="(_, name) in $slots" :slot="name" :name="name" />
        <!-- Pass on all scoped slots -->
        <template v-for="slot in Object.keys($scopedSlots)" #[slot]="scope"><slot :name="slot" v-bind="scope" /></template>
      </forge-multi-select>
      ...
      <div :style="{ height: height }" class="overflow-auto">
        <div class="border alternating-item-list">
          <div v-if="title" class="bg-selected-info p-2 border-top border-bottom shadow-sm bg-brand-dark">
            <span v-if="selectedItems.length === 0">No</span>
            <span>{{ title }}</span>
          </div>
