<template>
	<tr class="file-upload-table">
				...
				<b-row class="align-items-center ml-0">
					...
					<b-col cols="4">
						<div v-if="editableFileName">
							<forge-inline-editor :value="customFileName" :is-valid="validateFileName"
							                     @input="setFileName"></forge-inline-editor>
						</div>
						<div v-else>
		...
		<td class="text-center border-right-0">
			...
			<b-button variant="link" @click="uploadBlob" v-if="state === 'Not Uploaded'">
				<b-icon-upload />
			</b-button>
			<b-button variant="link" @click="uploadBlob" v-else-if="state === 'Failed' || state === 'Aborted'">
