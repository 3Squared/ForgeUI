        ...
        <template #header>
          <div class="d-flex justify-content-between pb-2">
            ...
            <div class="d-flex align-items-center">
              <slot name="re-render">
                <b-button v-if="rerender" class="mr-2" variant="primary" @click="$emit('re-render')">Re-render</b-button>
              </slot>
            ...
            <template v-for="(option, key) in props.options">
              <slot :name="key" v-bind="props.options">
                <div v-if="isSelect(getConfig(key))">
                  <label>{{ key }}</label>
                  <b-form-select v-model="props.options[key]" :options="getOptions(key)" class="mb-3" :disabled="isDisabled(key)" />
                </div>
                <b-form-checkbox v-else-if="typeof option === 'boolean'" v-model="props.options[key]" class="mb-2" :disabled="isDisabled(key)">
                  {{ key }}
                </b-form-checkbox>
                <template v-else-if="typeof option === 'number'">
                  <label>{{ key }}</label>
                  <b-form-input v-model.number="props.options[key]" class="mb-2" type="number" :disabled="isDisabled(key)" />
                </template>
                <template v-else-if="typeof option === 'string'">
                  <label>{{ key }}</label>
                  <b-form-input v-model="props.options[key]" class="mb-2" :disabled="isDisabled(key)" />
                </template>
              </slot>
            </template>
